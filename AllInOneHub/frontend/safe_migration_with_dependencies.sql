-- Safe Migration Script: Handle Dependencies Before Column Removal
-- This script safely removes dependencies before dropping columns
...
-- Step 14: Handle dependencies before removing columns
-- First, drop dependent policies
DO $$
BEGIN
  -- Drop departments policies that depend on auth_user_id
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'departments' 
    AND policyname = 'departments_insert_policy'
  ) THEN
    DROP POLICY IF EXISTS departments_insert_policy ON departments;
    RAISE NOTICE 'Dropped departments_insert_policy';
  END IF;

  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'departments' 
    AND policyname = 'departments_update_policy'
  ) THEN
    DROP POLICY IF EXISTS departments_update_policy ON departments;
    RAISE NOTICE 'Dropped departments_update_policy';
  END IF;

  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'departments' 
    AND policyname = 'departments_delete_policy'
  ) THEN
    DROP POLICY IF EXISTS departments_delete_policy ON departments;
    RAISE NOTICE 'Dropped departments_delete_policy';
  END IF;
END $$;
...
BEGIN
  IF EXISTS (
    ...
    WHERE viewname = 'user_auth_status'
  ) THEN
    DROP VIEW IF EXISTS user_auth_status;
    RAISE NOTICE 'Dropped user_auth_status view';
  END IF;
END $$;

-- Step 15: Now safely remove auth-related columns from employees
DO $$
BEGIN
  IF EXISTS (
...
-- Step 16: Add HR-specific columns to employees (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
...
-- Step 17: Update employee_id if needed
UPDATE employees 
SET employee_id = COALESCE(employee_id, 'EMP-' || LPAD(id::text, 6, '0'))
WHERE employee_id IS NULL;

-- Step 18: Create relationship view
...
-- Step 22: Show what was dropped
SELECT 
  'Dependencies Removed' as step,
  'The following objects were dropped due to auth_user_id dependency:' as description,
  'departments_insert_policy' as dropped_object,
  'Policy on departments table' as object_type
...
SELECT 
  'Dependencies Removed' as step,
  'The following objects were dropped due to auth_user_id dependency:' as description,
  'departments_update_policy' as dropped_object,
  'Policy on departments table' as object_type
...
SELECT 
  'Dependencies Removed' as step,
  'The following objects were dropped due to auth_user_id dependency:' as description,
  'departments_delete_policy' as dropped_object,
  'Policy on departments table' as object_type
...
SELECT 
  'Dependencies Removed' as step,
  'The following objects were dropped due to auth_user_id dependency:' as description,
  'user_auth_status' as dropped_object,
  'View' as object_type;
