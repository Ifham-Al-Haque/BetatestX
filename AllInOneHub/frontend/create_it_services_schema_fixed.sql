-- IT Services Database Schema - FIXED VERSION
-- This schema creates a comprehensive IT services management system
-- Fixed to work with existing database structure (users table instead of employees.role)
...
-- 5. Create IT Ticket Activities table for audit trail
...
-- 6. Create IT Assets table for asset requests
...
-- 7. Create IT Asset Assignments table for tracking asset history
...
-- 12. Create RLS Policies - FIXED to use users table
...
-- FIXED: Use users table instead of employees table for role check
...
CREATE INDEX IF NOT EXISTS idx_it_ticket_activities_user_id ON it_ticket_activities(user_id);
...
-- INSERT INTO it_requests (title, description, requester_id, category_id, priority_id) VALUES
-- ('Test Request 1', 'This is a test request for development', (SELECT id FROM users LIMIT 1), (SELECT id FROM it_request_categories LIMIT 1), (SELECT id FROM it_request_priorities LIMIT 1));

-- Success message
SELECT 'IT Services schema created successfully! All tables, policies, and functions are ready.' as status;
