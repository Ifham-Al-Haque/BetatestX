...
BEGIN
  ...
  IF keano_auth_user_id IS NOT NULL THEN
    ...
    -- Check if user exists in users table by email (more reliable)
    IF NOT EXISTS (SELECT 1 FROM public.users WHERE email = 'keano@udrive.ae') THEN
    ...
    ELSE
      RAISE NOTICE 'User already exists in users table, updating...';
      
      -- Update existing user record
      UPDATE public.users 
      SET auth_user_id = keano_auth_user_id,
      ...
      IF FOUND THEN
        RAISE NOTICE 'Updated existing user record with driver_management role';
      END IF;
    END IF;
  ELSE
...
SELECT 
  ...
  CASE 
    ...
    THEN 'Employee record exists (optional - only if keano is actually an employee)'
    ELSE 'No employee record (correct - not all users need to be employees)'
