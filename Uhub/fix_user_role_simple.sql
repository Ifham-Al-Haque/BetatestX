-- Fix User Role and Admin System (Simplified)
-- This script will fix the role issue and set up the admin system properly

-- 1. First, let's see what roles exist and their structure
SELECT '=== CURRENT ROLES TABLE STRUCTURE ===' as section;
SELECT column_name, data_type, is_nullable 
FROM information_schema.columns 
WHERE table_name = 'roles';

SELECT '=== CURRENT ROLES ===' as section;
SELECT * FROM roles;

-- 2. Add missing columns to roles table if they don't exist
DO $$ 
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'roles' AND column_name = 'access_level') THEN
        ALTER TABLE roles ADD COLUMN access_level INTEGER DEFAULT 1;
    END IF;
...
-- 3. Insert or update the specific roles requested
...
-- 5. CRITICAL: Update ifham@udrive.ae to have admin role
UPDATE employees 
SET role = 'admin', 
    role_id = (SELECT id FROM roles WHERE name = 'admin')
WHERE email = 'ifham@udrive.ae';

-- 6. Create invitations table for the invitation system
...
-- 7. Create invitation_roles table
...
-- 8. Insert default invitation roles
...
-- 9. Create invitation system functions
...
SELECT '=== ROLE FIX COMPLETE ===' as section;

SELECT 'Your updated admin status:' as info, 
    email, 
    role, 
    role_id,
    status
...
-- 13. Test invitation system
SELECT 'Test invitation token:' as info, generate_invitation_token() as sample_token;

-- 14. Show what should now be visible
SELECT 'Admin navigation should now show:' as info,
