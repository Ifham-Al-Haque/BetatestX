-- Safe Migration Script: Use CASCADE to Handle All Dependencies
-- This script safely removes all dependencies using CASCADE before dropping columns
...
-- Step 2: Check for dependencies on auth_user_id column
SELECT 
  'Dependency Check' as step,
  'Checking objects that depend on auth_user_id column' as description;

-- Check policies that depend on auth_user_id
SELECT 
  'Policy Dependencies' as dependency_type,
  schemaname,
  tablename,
  policyname,
  cmd,
  qual
FROM pg_policies 
WHERE qual::text LIKE '%auth_user_id%' 
   OR qual::text LIKE '%employees%';

-- Check views that depend on auth_user_id
SELECT 
  'View Dependencies' as dependency_type,
  schemaname,
  viewname,
  definition
FROM pg_views 
WHERE definition LIKE '%auth_user_id%' 
   OR definition LIKE '%employees%';

-- Check functions that depend on auth_user_id
SELECT 
  'Function Dependencies' as dependency_type,
  n.nspname as schema_name,
  p.proname as function_name,
  pg_get_functiondef(p.oid) as definition
FROM pg_proc p
JOIN pg_namespace n ON p.pronamespace = n.oid
WHERE pg_get_functiondef(p.oid) LIKE '%auth_user_id%'
   OR pg_get_functiondef(p.oid) LIKE '%employees%';
...
-- Step 14: Use CASCADE to drop columns and all dependencies automatically
DO $$
BEGIN
  -- Drop auth_user_id column with CASCADE to remove all dependencies
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'employees' 
    AND column_name = 'auth_user_id'
  ) THEN
    ALTER TABLE employees DROP COLUMN auth_user_id CASCADE;
    RAISE NOTICE 'Column auth_user_id removed from employees table with CASCADE';
  ELSE
    RAISE NOTICE 'Column auth_user_id already removed from employees table';
  END IF;

  -- Drop role column with CASCADE
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'employees' 
    ...
    RAISE NOTICE 'Column role removed from employees table with CASCADE';
  ELSE
    RAISE NOTICE 'Column role already removed from employees table';
  END IF;

  -- Drop status column with CASCADE
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'employees' 
    AND column_name = 'status'
  ) THEN
    ALTER TABLE employees DROP COLUMN status CASCADE;
    RAISE NOTICE 'Column status removed from employees table with CASCADE';
  ELSE
...
-- Step 21: Show what was automatically dropped by CASCADE
SELECT 
  'CASCADE Results' as step,
  'The following objects were automatically dropped by CASCADE:' as description,
  'All policies, views, and functions that depended on auth_user_id, role, and status columns' as dropped_objects,
  'This includes departments policies and user_auth_status view' as details;

-- Step 22: Check if departments table needs new policies
SELECT 
  'Post-Migration Check' as step,
  'Checking if departments table needs new RLS policies' as description;

-- Check if departments table has any policies
SELECT 
  'Current Departments Policies' as status,
  COUNT(*) as policy_count
FROM pg_policies 
WHERE tablename = 'departments';

-- Step 23: Provide guidance for recreating necessary policies
SELECT 
  'Next Steps' as step,
  'After successful migration, you may need to:' as description,
  '1. Recreate RLS policies for departments table' as action_1,
  '2. Update any application code that referenced dropped objects' as action_2,
  '3. Test the new user creation flow' as action_3;
