-- Create or repair public.expense_breakdowns for UHub (expenses.id is UUID).
-- Safe to run in Supabase SQL Editor if the table is missing or was created incorrectly.

DROP TABLE IF EXISTS public.expense_breakdowns CASCADE;

CREATE TABLE public.expense_breakdowns (
...
CREATE INDEX idx_expense_breakdowns_expense_id
  ON public.expense_breakdowns(expense_id);

CREATE INDEX idx_expense_breakdowns_expense_sort
