...
import { REGULARIZATION_TYPES } from './RegularizeAttendanceModal';

function typeLabel(value) {
  return REGULARIZATION_TYPES.find((t) => t.value === value)?.label || value;
}
...
const RegularizationInbox = ({ refreshKey = 0 }) => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await attendanceService.getRegularizationQueue(status);
      setRows(data);
      setSchemaMissing(false);
    } catch (err) {
      if (isAttendanceSchemaMissing(err)) setSchemaMissing(true);
      else showError('Regularization', err.message || 'Could not load requests');
    } finally {
      setLoading(false);
    }
  }, [status, showError]);
  ...
  const review = async (row, decision) => {
    ...
    setBusyId(row.id);
    try {
      const updated = await attendanceService.reviewRegularization(
        row.id,
        decision,
        notes[row.id] || ''
      );
      notificationService.notifyRegularizationReviewed(updated || row, decision).catch(() => {});
      success(
  ...
  if (schemaMissing) {
    return (
      <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-amber-900 text-sm">
        Regularization is not set up yet. Run{' '}
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-200 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold text-gray-900">Regularization inbox</h2>
        ...
        <table className="min-w-full text-sm">
          <thead className="bg-gray-50 text-xs uppercase text-gray-500">
            <tr>
              ...
              <th className="text-left px-6 py-3 font-medium">Requested times</th>
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan={6} className="px-6 py-8 text-center text-gray-500">
                  No {status === 'all' ? '' : status} regularization requests.
                </td>
              </tr>
            ) : (
              rows.map((r) => (
                <tr key={r.id} className="align-top">
                  <td className="px-6 py-3">
                    ...
                    {isUuid(r.employee_record_id) ? (
                      <Link to={`/employee/${r.employee_record_id}`} className="text-blue-600 text-xs hover:underline">
                        Employee profile
                      </Link>
                    ) : (
                  ...
                  <td className="px-6 py-3 whitespace-nowrap">{formatDubaiDate(r.work_date)}</td>
                  <td className="px-6 py-3 whitespace-nowrap">
                    <div>{typeLabel(r.request_type)}</div>
                    <div className="text-gray-500">
                      {(r.requested_clock_in || '—').toString().slice(0, 5)} → {(r.requested_clock_out || '—').toString().slice(0, 5)}
                    </div>
                  </td>
...
export default RegularizationInbox;
