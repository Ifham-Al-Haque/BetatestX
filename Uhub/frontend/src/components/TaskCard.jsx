...
const UserAvatar = ({ name, email, size = 'md' }) => {
  ...
  return (
    <div className={`${sizeClasses[size]} rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 text-white flex items-center justify-center font-semibold shadow-md ring-2 ring-white`}>
      {initials}
    </div>
  );
};
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1 pr-4">
            <div className="flex items-start gap-3 mb-3">
              <h3 className="text-xl font-bold text-gray-900 group-hover:text-blue-600 transition-colors cursor-pointer flex-1">
                {task.title}
              </h3>
              <div className="flex items-center gap-2 flex-shrink-0">
                {task.assignment_type === 'coordinated' && (
                  <span className="px-2.5 py-1 text-xs font-bold rounded-full bg-gradient-to-r from-purple-100 to-indigo-100 text-purple-800 border border-purple-200 flex items-center gap-1">
        ...
        <div className="space-y-3 mb-4 pb-4 border-b border-gray-100">
              ...
              <p className="text-sm font-semibold text-gray-900 truncate">
                {task.assignment_type === 'coordinated' && task.assignees && task.assignees.length > 0 ? (
                  <span className="flex items-center gap-2">
                    ...
                    <span className="text-purple-600 text-xs">({task.assignees.map(a => a.user_name).join(', ')})</span>
                  </span>
                ) : (
                  assignedToName
                )}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm font-semibold text-gray-900 truncate">{assignedByName}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div className="flex items-center gap-2 text-sm">
            ...
            <div className="min-w-0">
              ...
              <p className="text-sm font-medium text-gray-900 truncate">{task.department}</p>
            </div>
          </div>

          {task.due_date && (
            <div className="flex items-center gap-2 text-sm">
              <div className={`p-1.5 rounded-lg ${isOverdueTask ? 'bg-red-100' : 'bg-orange-100'}`}>
                <Calendar className={`w-3.5 h-3.5 ${isOverdueTask ? 'text-red-600' : 'text-orange-600'}`} />
              </div>
              <div className="min-w-0">
                <p className="text-xs text-gray-500">Due Date</p>
                <p className={`text-sm font-medium ${isOverdueTask ? 'text-red-600' : 'text-gray-900'}`}>
          ...
          {task.tags && task.tags.length > 0 && (
            <div className="flex items-center gap-1.5 flex-wrap">
              {task.tags.slice(0, 2).map((tag, index) => (
                <span 
                  key={index} 
                  className="px-2.5 py-1 bg-gradient-to-r from-gray-100 to-gray-50 text-gray-700 rounded-full text-xs font-medium border border-gray-200"
                >
              ...
              {task.tags.length > 2 && (
                <span className="text-xs text-gray-500 font-medium">+{task.tags.length - 2}</span>
              )}
            </div>
          )}
        </div>
